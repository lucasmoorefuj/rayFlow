...
describe("SessionContainer", function () {
    ...
    it("set/get/delete", function () {
        ...
        assert.equal(tokenString, originalTokenString, "Session token string must be equal to original header on initial set");
        ...
        assert.equal(tokenString, sameTokenString, "Session token string must be equal to the original higher version header");
        ...
        assert.equal(tokenStringWithNewerVersion, updatedTokenString, "Session token string must be equal to the new higher version header");
        ...
        assert(multiplePartitions.includes(tokenStringWithNewerVersion), "Token string must contain token from updated request");
        ...
        assert(multiplePartitions2.includes(tokenStringWithNewerVersion), "Token string must contain token from previous request for first partition");
        assert(multiplePartitions2.includes(p2TokenWithNewerVersion), "Token string must contain from updated token for second partition");
