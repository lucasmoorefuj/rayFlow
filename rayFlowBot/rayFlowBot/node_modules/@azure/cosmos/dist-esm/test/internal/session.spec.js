...
describe("New session token", function () {
    it("preserves tokens", async function () {
        ...
        const clientContext = sessionClient.clientContext;
        ...
        assert.equal(responseToken, rqContext === null || rqContext === void 0 ? void 0 : rqContext.headers["x-ms-session-token"]);
    });
});
...
async function createItem(container) {
    const { resource: { id } } = await container.items.create({
