    ...
    describe("#resolveServiceEndpoint", function () {
        it("should resolve the correct endpoint", async function () {
            const gem = new GlobalEndpointManager({
                ...
                const response = new ResourceResponse(new DatabaseAccount(databaseAccountBody, headers), headers, 200);
                return response;
            });
            // We don't block on init for database account calls
            assert.equal(await gem.resolveServiceEndpoint(ResourceType.none, OperationType.Read), "https://test.documents.azure.com:443/");
            assert.equal(await gem.resolveServiceEndpoint(ResourceType.item, OperationType.Read), "https://test-eastus2.documents.azure.com:443/");
        });
...
//# sourceMappingURL=globalEndpointManager.spec.js.map