...
describe("Cross Partition", function () {
    ...
    describe("Validate Query", function () {
        ...
        const validateResults = function (actualResults, expectedOrderIds, expectedCount) {
            assert.equal(actualResults.length, expectedCount ||
                (expectedOrderIds && expectedOrderIds.length) ||
                documentDefinitions.length, "actual results length doesn't match with expected results length.");
            if (expectedOrderIds) {
                assert.deepStrictEqual(actualResults.map((doc) => doc.id || doc), expectedOrderIds);
            }
        };
        const validateFetchAll = async function (queryIterator, options, expectedOrderIds, expectedCount) {
            ...
            assert.equal(results.length, expectedCount ||
                (expectedOrderIds && expectedOrderIds.length) ||
                documentDefinitions.length, "invalid number of results");
        ...
        const validateFetchNextAndHasMoreResults = async function (options, queryIterator, expectedOrderIds, fetchAllResponse, expectedCount, expectedIteratorCalls) {
            const pageSize = options["maxItemCount"];
            let totalExecuteNextRequestCharge = 0;
            let totalIteratorCalls = 0;
            let totalFetchedResults = [];
            const expectedLength = expectedCount ||
                (expectedOrderIds && expectedOrderIds.length) ||
            ...
            const percentDifference = Math.abs(fetchAllResponse.requestCharge - totalExecuteNextRequestCharge) /
        ...
        const validateAsyncIterator = async function (queryIterator, expectedOrderIds, expecetedCount) {
            var e_1, _a;
            const expectedLength = expecetedCount ||
                (expectedOrderIds && expectedOrderIds.length) ||
                documentDefinitions.length;
            const results = [];
            let completed = false;
        ...
        const executeQueryAndValidateResults = async function ({ query, options, expectedOrderIds, expectedCount, expectedRus, expectedIteratorCalls }) {
            ...
            if (expectedRus) {
                ...
                assert(percentDifference <= 0.05, `difference between fetchAll request charge and expected request charge should be less than 5%. Got ${percentDifference *
                    100}`);
            }
            queryIterator.reset();
            await validateFetchNextAndHasMoreResults(options, queryIterator, expectedOrderIds, fetchAllResponse, expectedCount, expectedIteratorCalls);
            queryIterator.reset();
