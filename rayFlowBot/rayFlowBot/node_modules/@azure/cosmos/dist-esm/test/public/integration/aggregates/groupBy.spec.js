...
describe("Cross partition GROUP BY", () => {
    ...
    before(async () => {
        ...
        container = await getTestContainer("GROUP BY Query", undefined, containerDefinition, containerOptions);
    ...
    it("with multiple fields", async () => {
        const queryIterator = container.items.query("SELECT c.age, c.name FROM c GROUP BY c.age, c.name", options);
        const result = await queryIterator.fetchAll();
        snapshot(result.resources.sort((a, b) => a.age - b.age || a.name.localeCompare(b.name)));
    });
    it("with COUNT", async () => {
        const queryIterator = container.items.query("SELECT c.age, COUNT(1) as count FROM c GROUP BY c.age", options);
        const result = await queryIterator.fetchAll();
        snapshot(result.resources.sort((a, b) => a.age - b.age));
    });
    it("with MIN", async () => {
        const queryIterator = container.items.query("SELECT c.name, MIN(c.age) AS min_age FROM c GROUP BY c.name", options);
        const result = await queryIterator.fetchAll();
        snapshot(result.resources.sort((a, b) => a.name.localeCompare(b.name)));
    });
    it("with MAX", async () => {
        const queryIterator = container.items.query("SELECT c.name, MAX(c.age) AS min_age FROM c GROUP BY c.name", options);
        const result = await queryIterator.fetchAll();
        snapshot(result.resources.sort((a, b) => a.name.localeCompare(b.name)));
    });
    it("with SUM", async () => {
        const queryIterator = container.items.query("SELECT c.name, SUM(c.age) AS min_age FROM c GROUP BY c.name", options);
        const result = await queryIterator.fetchAll();
        snapshot(result.resources.sort((a, b) => a.name.localeCompare(b.name)));
    });
    it("with AVG", async () => {
        const queryIterator = container.items.query("SELECT c.name, AVG(c.age) AS min_age FROM c GROUP BY c.name", options);
        const result = await queryIterator.fetchAll();
        snapshot(result.resources.sort((a, b) => {
            return a.name.localeCompare(b.name);
        }));
    });
    it("with multiple aggregates", async () => {
        const queryIterator = container.items.query("SELECT c.name, Count(1) AS count, Min(c.age) AS min_age, Max(c.age) AS max_age FROM c GROUP BY c.name", options);
        const result = await queryIterator.fetchAll();
        snapshot(result.resources.sort((a, b) => {
            return a.name.localeCompare(b.name);
        }));
    ...
    it("with VALUE with string", async () => {
        const queryIterator = container.items.query("SELECT VALUE c.team FROM c GROUP BY c.team", options);
        const result = await queryIterator.fetchAll();
        snapshot(result.resources.sort());
    });
    it("with VALUE with number", async () => {
        const queryIterator = container.items.query("SELECT VALUE c.age FROM c GROUP BY c.age", options);
        const result = await queryIterator.fetchAll();
        snapshot(result.resources.sort());
    });
    it("with VALUE and aggregate", async () => {
        const queryIterator = container.items.query("SELECT VALUE AVG(c.age) FROM c GROUP BY c.team", options);
        const result = await queryIterator.fetchAll();
        snapshot(result.resources.sort());
    });
    it("with aggregates and fields that do not exist", async () => {
        const queryIterator = container.items.query(`SELECT
        c.age,
      ...
      FROM c
      GROUP BY c.age`, options);
        const result = await queryIterator.fetchAll();
        snapshot(result.resources.sort((a, b) => a.age - b.age));
    });
    it("with missing aggregate field", async () => {
        const queryIterator = container.items.query('SELECT AVG("asdf") as avg_asdf FROM c GROUP BY c.age', options);
        const result = await queryIterator.fetchAll();
        snapshot(result.resources.sort((a, b) => a.age - b.age));
    });
    it("with missing GROUP BY projection", async () => {
        const queryIterator = container.items.query("SELECT c.age, c.doesNotExist FROM c GROUP BY c.age, c.doesNotExist", options);
        const result = await queryIterator.fetchAll();
        snapshot(result.resources.sort((a, b) => a.age - b.age));
    });
    ...
    it("works with TOP", async () => {
        const queryIterator = container.items.query("SELECT TOP 1 c.age FROM c GROUP BY c.age", options);
        const result = await queryIterator.fetchNext();
