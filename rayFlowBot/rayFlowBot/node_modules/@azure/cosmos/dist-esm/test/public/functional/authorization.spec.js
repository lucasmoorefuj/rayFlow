...
describe("NodeJS CRUD Tests", function () {
    ...
    describe("Validate Authorization", function () {
        it("should handle all the key options", async function () {
            ...
            assert(undefined !== (await clientOptionsKey.databases.readAll().fetchAll()), "Should be able to fetch list of databases");
            const clientOptionsAuthKey = new CosmosClient({
                endpoint,
                key: masterKey,
                connectionPolicy: { enableBackgroundEndpointRefreshing: false }
            });
            assert(undefined !== (await clientOptionsAuthKey.databases.readAll().fetchAll()), "Should be able to fetch list of databases");
            const clientOptionsAuthMasterKey = new CosmosClient({
                endpoint,
                key: masterKey,
                connectionPolicy: { enableBackgroundEndpointRefreshing: false }
            });
            assert(undefined !== (await clientOptionsAuthMasterKey.databases.readAll().fetchAll()), "Should be able to fetch list of databases");
        });
        // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
        const setupEntities = async function (isUpsertTest) {
            // create database
            ...
            const { resource: permissionOnColl1 } = await createOrUpsertPermission(database.user(user1.id), permission, undefined, isUpsertTest);
            ...
            const { resource: permissionOnDoc2 } = await createOrUpsertPermission(database.user(user1.id), permission, undefined, isUpsertTest);
            ...
            const { resource: permissionOnColl2 } = await createOrUpsertPermission(database.user(user2.id), permission, undefined, isUpsertTest);
            const entities = {
                database,
                coll1: container1,
                coll2: container2,
        ...
        const authorizationCRUDTest = async function (isUpsertTest) {
            try {
        ...
        it("should allow deletion of a doc with container token", async function () {
            ...
            // Create Permission
            const { resource: permission } = await createOrUpsertPermission(container.database.user(user.id), {
                id: "permission On Coll1",
                permissionMode: PermissionMode.All,
