...
import { getTestDatabase, removeAllDatabases, getTestContainer, assertThrowsAsync } from "../common/TestHelpers";
...
describe("Containers", function () {
    ...
    describe("Container CRUD", function () {
        const containerCRUDTest = async function (partitionKey) {
            // create database
            ...
            if (containerDef.partitionKey) {
                const comparePaths = typeof containerDefinition.partitionKey === "string"
    ...
    describe("Indexing policy", function () {
        it("Create container with correct indexing policy", async function () {
            ...
            const container = database.container(containerDef.id);
            assert.equal(containerDef.indexingPolicy.indexingMode, IndexingMode.consistent, "default indexing mode should be consistent");
            await container.delete();
            ...
            assert.equal(containerDef.indexingPolicy.indexingMode, IndexingMode.consistent, "indexing mode should be consistent");
            ...
            assert.equal(2, containerWithIndexingPolicyDef.indexingPolicy.excludedPaths.length, "Unexpected excludedPaths length");
    ...
    describe("Validate response headers", function () {
        const createThenReadcontainer = async function (database, definition) {
...
describe("container.readOffer", function () {
    let containerWithOffer;
    let containerWithoutOffer;
    let container2WithOffer;
    let container2WithoutOffer;
    const containerRequestWithOffer = {
        id: "sample",
        throughput: 400
    };
    ...
    before(async function () {
        ...
        containerWithOffer = await getTestContainer("offerContainer", undefined, containerRequestWithOffer);
    ...
    describe("database does not have offer", function () {
        it("has offer", async function () {
            const offer = await containerWithOffer.readOffer();
        ...
        it("does not have offer so uses default", async function () {
            const offer = await containerWithoutOffer.readOffer();
    ...
    describe("database has offer", function () {
        it("container does not have offer", async function () {
            const offer = await container2WithoutOffer.readOffer();
