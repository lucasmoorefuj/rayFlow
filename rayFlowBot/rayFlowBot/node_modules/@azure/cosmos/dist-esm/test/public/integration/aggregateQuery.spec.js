...
describe("Aggregate Query", function () {
    ...
    const samePartitionSum = (testdata.numberOfDocsWithSamePartitionKey * (testdata.numberOfDocsWithSamePartitionKey + 1)) /
        2.0;
    let container;
    ...
    before(async function () {
        ...
        container = await getTestContainer("Validate Aggregate Document Query", undefined, containerDefinition, containerOptions);
        await bulkInsertItems(container, documentDefinitions);
    });
    const validateFetchAll = async function (queryIterator, expectedResults) {
    ...
    const validateExecuteNextAndHasMoreResults = async function (queryIterator, options, expectedResults, fetchAllRequestCharge) {
        ...
        const percentDifference = Math.abs(fetchAllRequestCharge - totalExecuteNextRequestCharge) /
            totalExecuteNextRequestCharge;
        assert(percentDifference <= 0.01, "difference between fetchAll request charge and executeNext request charge should be less than 1%");
    };
    const ValidateAsyncIterator = async function (queryIterator, expectedResults) {
        var e_1, _a;
        const results = [];
        let completed = false;
        try {
            // forEach uses callbacks still, so just wrap in a promise
            for (var _b = __asyncValues(queryIterator.getAsyncIterator()), _c; _c = await _b.next(), !_c.done;) {
                const { resources: items } = _c.value;
    ...
    const executeQueryAndValidateResults = async function (query, expectedResults) {
        const options = { maxDegreeOfParallelism: 2, maxItemCount: 1 };
    ...
    it("SELECT VALUE AVG with ORDER BY", async function () {
        await executeQueryAndValidateResults("SELECT VALUE AVG(r.key) FROM r WHERE IS_NUMBER(r.key) ORDER BY r.key", [average]);
    });
    ...
    it("SELECT VALUE SUM with ORDER BY", async function () {
        await executeQueryAndValidateResults("SELECT VALUE SUM(r.key) FROM r WHERE IS_NUMBER(r.key) ORDER BY r.key", [testdata.sum]);
    });
    it("SELECT VALUE AVG for single partiton", async function () {
        await executeQueryAndValidateResults("SELECT VALUE AVG(r.field) FROM r WHERE r.key = 'uniquePartitionKey'", [samePartitionSum / testdata.numberOfDocsWithSamePartitionKey]);
    });
    it("SELECT VALUE COUNT for single partiton", async function () {
        await executeQueryAndValidateResults("SELECT VALUE COUNT(r.field) FROM r WHERE r.key = 'uniquePartitionKey'", [testdata.numberOfDocsWithSamePartitionKey]);
    });
    it("SELECT VALUE MAX for single partiton", async function () {
        await executeQueryAndValidateResults("SELECT VALUE MAX(r.field) FROM r WHERE r.key = 'uniquePartitionKey'", [testdata.numberOfDocsWithSamePartitionKey]);
    });
    it("SELECT VALUE MIN for single partiton", async function () {
        await executeQueryAndValidateResults("SELECT VALUE MIN(r.field) FROM r WHERE r.key = 'uniquePartitionKey'", [1]);
    });
    it("SELECT VALUE SUM for single partiton", async function () {
        await executeQueryAndValidateResults("SELECT VALUE SUM(r.field) FROM r WHERE r.key = 'uniquePartitionKey'", [samePartitionSum]);
    });
