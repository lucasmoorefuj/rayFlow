...
// Copyright (c) Microsoft Corporation.
/** @hidden */
const logger$1 = logger$4.createClientLogger("parallelQueryExecutionContextBase");
/** @hidden */
    ...
    constructor(clientContext, collectionLink, query, options, partitionedQueryExecutionInfo) {
        ...
        const createDocumentProducersAndFillUpPriorityQueueFunc = async () => {
            ...
            try {
                ...
                logger$1.info("Query starting against " +
                    targetPartitionRanges.length +
...
// Copyright (c) Microsoft Corporation.
const logger$2 = logger$4.createClientLogger("RequestHandler");
...
async function httpRequest(requestContext) {
    ...
    if (response.status >= 400) {
        ...
        logger$2.warning(response.status +
            " " +
...
class ClientContext {
    ...
    async queryFeed({ path, resourceType, resourceId, resultFn, query, options, partitionKeyRangeId, partitionKey }) {
        ...
        logger$3.verbose(request$1);
        const start = Date.now();
        const response = await request(request$1);
        logger$3.info("query " + requestId + " finished - " + (Date.now() - start) + "ms");
